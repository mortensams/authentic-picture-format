export class ExifExtractor {
  static EXIF_TAGS = {
    ...
    F_NUMBER: 0x829D,
    APERTURE_VALUE: 0x9202,  // ApertureValue - actual aperture value in APEX units
    FOCAL_LENGTH: 0x920A,
    FOCAL_LENGTH_IN_35MM: 0xA405,  // FocalLengthIn35mmFilm
    GPS_IFD: 0x8825,
    LENS_MAKE: 0xA433,
    LENS_MODEL: 0xA434,
    IMAGE_WIDTH: 0x0100,
  ...
  static readTagValue(dataView, tag, type, count, valueOffset, littleEndian, tiffOffset) {
    switch (tag) {
      ...
      case this.EXIF_TAGS.APERTURE_VALUE:
      case this.EXIF_TAGS.FOCAL_LENGTH:
      case this.EXIF_TAGS.FOCAL_LENGTH_IN_35MM:
  ...
  static formatExifData(ifdData) {
    ...
    // Format aperture - try F_NUMBER first, then APERTURE_VALUE
    let aperture = ifdData[this.EXIF_TAGS.F_NUMBER];
    console.log('F_NUMBER tag value:', aperture);
    
    // If F_NUMBER is invalid, try ApertureValue directly
    if (!aperture || aperture <= 0 || aperture >= 100) {
      const apertureValue = ifdData[this.EXIF_TAGS.APERTURE_VALUE];
      console.log('ApertureValue tag value:', apertureValue);
      
      if (apertureValue !== null && apertureValue !== undefined && apertureValue > 0 && apertureValue < 100) {
        // Just use the ApertureValue directly as the f-number
        aperture = apertureValue;
        console.log('Using ApertureValue directly as f/', aperture);
      }
    }
    
    if (aperture && aperture > 0 && aperture < 100) { // Sanity check
      formatted.aperture = aperture;
      formatted.apertureString = `f/${aperture.toFixed(1)}`;
      console.log('Final aperture:', formatted.apertureString);
    } else {
      console.log('Aperture failed sanity check or not found:', aperture);
    }
    ...
    // Format focal length - try FOCAL_LENGTH first, then FOCAL_LENGTH_IN_35MM
    let focalLength = ifdData[this.EXIF_TAGS.FOCAL_LENGTH];
    
    // Validate focal length
    if (!focalLength || focalLength <= 0.1 || focalLength >= 10000) {
      // Try 35mm equivalent as fallback
      const focalLength35mm = ifdData[this.EXIF_TAGS.FOCAL_LENGTH_IN_35MM];
      if (focalLength35mm && focalLength35mm > 0) {
        console.log('Using FocalLengthIn35mmFilm:', focalLength35mm);
        focalLength = focalLength35mm;
      }
    }
    
    if (focalLength && focalLength > 0.1 && focalLength < 10000) { // Sanity check
      formatted.focalLength = focalLength;
      formatted.focalLengthString = `${Math.round(focalLength)}mm`;
    }
    
    // For iPhone, parse lens string as fallback for focal length and aperture
    // Format: "Apple iPhone 16 Pro back triple camera 2.22mm f/2.2"
    if (formatted.lens && (!formatted.focalLength || !formatted.aperture)) {
      const lensMatch = formatted.lens.match(/(\d+\.?\d*)mm\s+f\/(\d+\.?\d*)/);
      if (lensMatch) {
        if (!formatted.focalLength) {
          formatted.focalLength = parseFloat(lensMatch[1]);
          formatted.focalLengthString = `${formatted.focalLength}mm`;
          console.log('Extracted focal length from lens string:', formatted.focalLengthString);
        }
        if (!formatted.aperture) {
          formatted.aperture = parseFloat(lensMatch[2]);
          formatted.apertureString = `f/${formatted.aperture}`;
          console.log('Extracted aperture from lens string:', formatted.apertureString);
        }
      }
    }
